class KinService {
    ...
    async importWallet(secretkey , password) {
        let msg = {
            from: 'popup',
            type: 'WALLET_IMPORT_REQUEST',
            data: {
                secretkey: secretkey,
                password: password
            }
    ...
    async transfer(address, amount) {
        ...
        let promise = new Promise((resolve, reject) => {
            chrome.runtime.sendMessage(msg, (res) => {
                if(res.error && Object.getOwnPropertyNames(res.error).length !== 0)
                    reject(res.error)
    ...
    async notifyInpageTransferSuccess(id, address, amount) {
        let msg = {
            from: 'popup',
